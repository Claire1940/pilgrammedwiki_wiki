...
// 静态页面配置
const staticPagesConfig: Record<
  string,
  { priority: number; changeFrequency: "monthly" | "yearly" }
> = {
...
// 内容更新频率配置
const contentTypeChangeFrequency: Record<
  string,
      ...
      try {
        ...
        for (const article of articles) {
          ...
          const changeFrequency =
            contentTypeChangeFrequency[contentType] || "weekly";
        ...
        // 忽略无法加载的内容类型
        console.warn(
          `Failed to load content for ${locale}/${contentType}:`,
          error,
        );
      }
    }
